function [wynik]=spr_d(jacoby,odnosnik,uklad,k_max,n,d,d_iter,rodzaj,b_rand,A_rand,A_rand_d)
 %% spr_d
 % funkcja przyjmuje:
 % jacoby - @jacoby
 % odnonnik - @odnosnik
 ...
 % d_iter - liczba iteracji funkcji spr_d(ile razy ma sprawdzic uklad z danym parametrem definujacym dokladnosc)
 ...
 % funkcja zwraca:
 % wynik(row,1)=d(1,k) - parametr definiujacy dokladnosc z ktorym wywołuje funkcje @jacoby
...
wynik=zeros(length(d)*d_iter,4);
for iter=1:d_iter
    for k=1:length(d)
        A_b=uklad(rodzaj,n,b_rand,A_rand,A_rand_d);
        A=A_b(:,1:n);
        b=A_b(:,n+1);
        j=jacoby(A,b,d(1,k),k_max);
        odn=odnosnik(A,b);
        row=(iter-1)*length(d)+k;
        wynik(row,1)=d(1,k);
        wynik(row,2)=j(1,1);
